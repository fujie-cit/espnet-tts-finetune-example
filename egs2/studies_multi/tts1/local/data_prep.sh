...
for name in Emotion100-Happy Emotion100-Angry Emotion100-Sad Emotion100-Normal Recitation324; do
    ...
    transcript_filename=`echo ${db}/${spk}/${name}/txt/transcript*.txt`
    python local/make_text.py ${transcript_filename} ${scp} > ${text}
    echo "finished making text."

    # make segments
    find ${db}/${spk}/${name}/full -follow -name "*.lab" | sort | while read -r filename; do
